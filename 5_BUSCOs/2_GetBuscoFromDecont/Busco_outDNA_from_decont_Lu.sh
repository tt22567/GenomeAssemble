...
DIR_genomes=/user/work/tt22567/Neuroptera/4_trifasta/4_outDnaTri  #2_inDnaTri,4_outDnaTri
...
#ingenome,outgenome
DIR_output=./DIR_Busco_outgenome
DIR_csvfile=./DIR_Busco_csv
log_file=busco_outgenome_$ref_species.log
csv_file=summarizeBusco_outgenome_$ref_species.csv
...
for i in $DIR_genomes/*_scaffolds.fasta;do
  ...
  if [ ! -d $DIR_output/$od ] || [ ! -f $DIR_output/$od/$txt ]; then
	...
	echo "Busco assessment and geting single-copy othology genes of $i finished" >> $DIR_logfile/$log_file
  fi
done
...
for document in $DIR_output/*$db_name;do 
  ...
  if [ ! -d $gene_file/$od ];then
	...
	cp -r $gen  $gene_file/$od  2>&1 && rm -rf $gene_file/$od/*fna && #outgroup exists both faa and fna fasta.
