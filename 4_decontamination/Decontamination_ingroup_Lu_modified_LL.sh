#!/bin/sh
#SBATCH --job-name=in_Decont
#SBATCH --partition=cpu
#SBATCH --output=./inrnatri_%A.out
#SBATCH --nodes=1
...
cd /user/work/tt22567/Neuroptera/3_tridecont
...
SeqOrinPiePlot=user/work/tt22567/Neuroptera/3_tridecont/SeqOriginPiePlot.py #path to script for drawing
...
DIR_asm=$DIR_cur/1_inRnaTri  #path to tri.fasta
DIR_sif=$DIR_cur/in_Fasta  #$rnatri.Sifted.fasta is the seq after removing vector; $rnatri.decontX.fasta is the seq after decontamination;$rnatri.Sifted.DeContX.L.fasta is the seq after seeking for the longest isoform.
DIR_dec=$DIR_cur/in_Outfmt  #$rnatri.decont is the csv get from blastn；$rnatri.decontX is the csv from diamond
DIR_tdc=$DIR_cur/in_Faa  #$rnatri.Trans.DX.L.pep.faa is the protein seq after searching ORF by transdecoder；$rnatri.Trans.DX.L.pep.cdhit.faa is the seq after removing duplication, which will be used for BUSCO。
DIR_Trc=$DIR_cur/in_TranCont  #component analysis $rnatri.decontX.csv
...
InputFasta=$(ls ./1_inRnaTri/*_trinity.Trinity.fasta | sed -e "s/.fasta//g;s/.\/1_inRnaTri\///g") # all input fasta files  #output=speciesname_rna.trinity.Trinity, such as Xanthostigma_gobicola_SRR1811918.rna_trinity.Trinity
NNN=$(echo $InputFasta | wc -w)  #统计数量

EcdysFasta=$(ls ./1_inRnaTri/*_trinity.Trinity.fasta | sed -e "s/.fasta//g;s/.\/1_inRnaTri\///g" | grep -E "^Xenos|^Mengenilla|^Stylops") # Names of those (from Strepsiptera) should be decontaminated by searching Holometabola genes, such as Mengenilla_moldrzyki_SRR619393.rna_trinity.Trinity
...
runDeVect(){
    ...
    if [[ $EcdysFasta =~ $rnatri ]]; then  #包含
        local DeConProg=/user/work/tt22567/Neuroptera/3_tridecont/DeContaminRNA-tx.py
    else
        local DeConProg=/user/work/tt22567/Neuroptera/3_tridecont/DeContaminRNA-rk.py  
    fi
...
chmod -R 777 /user/work/tt22567/Neuroptera/3_tridecont/*.py 
...
#source /sw/languages/anaconda/anaconda.3.9-2021.12-bioconda/bin/activate    /user/home/tt22567/.conda/envs/Lacewings #transdecoder install in Lacewings, together with trinity
